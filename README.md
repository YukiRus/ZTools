...
- 因为是AI CODING未经人工审查，所以自用就好了，免得给原作者带来困扰。
- 或许也有可能别人不喜欢我的改动呢？

### 2026-02-15
- 新增关键字匹配历史记录（每个关键词都会保存前5个搜索记录置顶）
- 新增匹配lnk指向的文件名
- 新增匹配lnk的本地化/原始命名（如Remote Desktop Connection/远程桌面连接 两个名字均可以用于匹配）
- FIX: 英文缩写匹配有空格也能正确匹配了 （如 RDC -> Remote Desktop Connection）
